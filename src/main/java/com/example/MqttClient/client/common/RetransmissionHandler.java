...
final class RetransmissionHandler<T extends MqttMessage> {
    ...
    private void startTimer(EventLoop eventLoop){
        this.timer = eventLoop.schedule(() -> {
            this.timeout += 5;
            MqttFixedHeader fixedHeader = new MqttFixedHeader(this.originalMessage.fixedHeader().messageType(), true, this.originalMessage.fixedHeader().qos(), this.originalMessage.fixedHeader().retain(), 0);
