...
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttDecoder;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttEncoder;
...
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttPublishVariableHeader;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttQoS;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttSubscribeMessage;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttSubscribePayload;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttTopicSubscription;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttUnsubscribeMessage;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttUnsubscribePayload;
...
public final class MqttClientImpl implements MqttClient {
    ...
    public Future<Void> publish(String topic, ByteBuf payload, MqttQoS qos, boolean retain) {
        ...
        MqttPublishVariableHeader variableHeader = new MqttPublishVariableHeader(topic, getNewMessageId().packetId());
    ...
    private MqttPacketIdVariableHeader getNewMessageId() {
        this.nextMessageId.compareAndSet(0xffff, 1);
        return MqttPacketIdVariableHeader.from(this.nextMessageId.getAndIncrement());
    }
    ...
    private Future<Void> createSubscription(String topic, MqttHandler handler, boolean once, MqttQoS qos) {
        ...
        MqttPacketIdVariableHeader variableHeader = getNewMessageId();
        ...
        this.pendingSubscriptions.put(variableHeader.packetId(), pendingSubscription);
    ...
    private void checkSubscribtions(String topic, Promise<Void> promise) {
        if (!(this.subscriptions.containsKey(topic) && this.subscriptions.get(topic).size() != 0) && this.serverSubscriptions.contains(topic)) {
            ...
            this.pendingServerUnsubscribes.put(variableHeader.packetId(), pendingUnsubscription);
