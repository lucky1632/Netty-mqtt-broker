...
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttConnAckMessage;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttConnectMessage;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttConnectPayload;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttConnectReturnCode;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttConnectVariableHeader;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttFixedHeader;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttMessage;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttMessageType;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttPacketIdVariableHeader;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttPublishMessage;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttQoS;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttSubAckMessage;
...
final class MqttChannelHandler extends SimpleChannelInboundHandler<MqttMessage> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, MqttMessage msg) throws Exception {
        switch (msg.fixedHeader().messageType()) {
            ...
            case PUBLISH:
                ...
                break;
            case UNSUBACK:
                handleUnsuback((MqttMessage) msg);
                break;
            case PUBACK:
                handlePuback((MqttMessage) msg);
                break;
            case PUBREC:
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        MqttConnectVariableHeader variableHeader = new MqttConnectVariableHeader(
                ...
                this.client.getClientConfig().getLastWill() != null                 // Will QOS
                        ? MqttQoS.valueOf(this.client.getClientConfig().getLastWill().getQos().value())
        ...
        MqttConnectPayload payload = new MqttConnectPayload(
                ...
                this.client.getClientConfig().getLastWill() != null ? String.valueOf(this.client.getClientConfig().getLastWill().getMessage().getBytes(CharsetUtil.UTF_8)) : null,
                this.client.getClientConfig().getUsername(),
                this.client.getClientConfig().getPassword() != null ? String.valueOf(this.client.getClientConfig().getPassword().getBytes(CharsetUtil.UTF_8)) : null
        );
    ...
    private void handleConack(Channel channel, MqttConnAckMessage message) {
        switch (message.variableHeader().returnCode()) {
            ...
            case CONNECTION_REFUSED_UNACCEPTABLE_PROTOCOL_VERSION:
                this.connectFuture.setSuccess(new MqttConnectResult(false, message.variableHeader().returnCode(), channel.closeFuture()));
                channel.close();
    ...
    private void handleSubAck(MqttSubAckMessage message) {
        MqttPendingSubscription pendingSubscription = this.client.getPendingSubscriptions().remove(message.variableHeader().packetId());
        ...
        switch (message.fixedHeader().qos()) {
            ...
            case AT_LEAST_ONCE:
                ...
                if (message.variableHeader().packetId() != -1) {
                    ...
                    MqttPacketIdVariableHeader variableHeader = MqttPacketIdVariableHeader.from(message.variableHeader().packetId());
//                    channel.writeAndFlush(new MqttPubAckMessage(fixedHeader, variableHeader));
    ...
    private void handleUnsuback(MqttMessage message) {
        MqttPendingUnsubscription unsubscription = this.client.getPendingServerUnsubscribes().get(((MqttPacketIdVariableHeader)message.variableHeader()).packetId());
        ...
        this.client.getPendingServerUnsubscribes().remove(((MqttPacketIdVariableHeader)message.variableHeader()).packetId());
    }

    private void handlePuback(MqttMessage message) {
        MqttPendingPublish pendingPublish = this.client.getPendingPublishes().get(((MqttPacketIdVariableHeader)message.variableHeader()).packetId());
        pendingPublish.getFuture().setSuccess(null);
        pendingPublish.onPubackReceived();
        this.client.getPendingPublishes().remove(((MqttPacketIdVariableHeader)message.variableHeader()).packetId());
    ...
    private void handlePubrel(Channel channel, MqttMessage message) {
        if (this.client.getQos2PendingIncomingPublishes().containsKey(((MqttPacketIdVariableHeader) message.variableHeader()).packetId())) {
            MqttIncomingQos2Publish incomingQos2Publish = this.client.getQos2PendingIncomingPublishes().get(((MqttPacketIdVariableHeader) message.variableHeader()).packetId());
        ...
        MqttPacketIdVariableHeader variableHeader = MqttPacketIdVariableHeader.from(((MqttPacketIdVariableHeader) message.variableHeader()).packetId());
        channel.writeAndFlush(new MqttMessage(fixedHeader, variableHeader));
    }

    private void handlePubcomp(MqttMessage message) {
        MqttPacketIdVariableHeader variableHeader = (MqttPacketIdVariableHeader) message.variableHeader();
        MqttPendingPublish pendingPublish = this.client.getPendingPublishes().get(variableHeader.packetId());
        pendingPublish.getFuture().setSuccess(null);
        this.client.getPendingPublishes().remove(variableHeader.packetId());
        pendingPublish.getPayload().release();
