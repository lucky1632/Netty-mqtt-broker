...
package com.example.MqttClient.io.netty.handler.codec.mqtt;

import io.netty.buffer.ByteBuf;
import io.netty.channel.ChannelHandlerContext;
import io.netty.handler.codec.DecoderException;
import io.netty.handler.codec.ReplayingDecoder;
import com.example.MqttClient.io.netty.handler.codec.mqtt.MqttDecoder.DecoderState;
import io.netty.util.CharsetUtil;
import java.util.ArrayList;
import java.util.List;

/**
 * Decodes Mqtt messages from bytes, following <a
 * href="http://public.dhe.ibm.com/software/dw/webservices/ws-mqtt/mqtt-v3r1.html">the MQTT protocol
 * specification v3.1</a>
 */
public class MqttDecoder extends ReplayingDecoder<DecoderState> {

  private static final int DEFAULT_MAX_BYTES_IN_MESSAGE = 0x7fffffff;
  ...
  /**
   * Decodes the fixed header. It's one byte for the flags and then variable bytes for the remaining
   * length.
   *
  ...
  private static MqttFixedHeader decodeFixedHeader(ByteBuf buffer) {
    ...
    return new MqttFixedHeader(
        messageType, dupFlag, MqttQoS.valueOf(qosLevel), retain, remainingLength);
  }
  ...
  private static Result<MqttConnectVariableHeader> decodeConnectionVariableHeader(ByteBuf buffer) {
    ...
    final MqttVersion mqttVersion =
        MqttVersion.fromProtocolNameAndLevel(protocolName.value, protocolLevel);
    ...
    final MqttConnectVariableHeader mqttConnectVariableHeader =
  ...
  private static Result<MqttPacketIdVariableHeader> decodePacketIdVariableHeader(ByteBuf buffer) {
    ...
    return new Result<>(
        MqttPacketIdVariableHeader.from(packetId.value), packetId.numberOfBytesConsumed);
  }
  ...
  private static Result<MqttConnectPayload> decodeConnectionPayload(
      ByteBuf buffer, MqttConnectVariableHeader mqttConnectVariableHeader) {
  ...
  private static Result<MqttSubscribePayload> decodeSubscribePayload(
      ByteBuf buffer, int bytesRemainingInVariablePart) {
  ...
  private static Result<ByteBuf> decodePublishPayload(
      ByteBuf buffer, int bytesRemainingInVariablePart) {
    ...
    switch (state()) {
        ...
        try {
          if (bytesRemainingInVariablePart > maxBytesInMessage) {
            throw new DecoderException(
                "message size exceeds limit: " + bytesRemainingInVariablePart + " bytes");
          }
      ...
      case READ_PAYLOAD:
        try {
          ...
          MqttMessage message =
              MqttMessageFactory.newMessage(mqttFixedHeader, variableHeader, payload);
  ...
  /**
   * States of the decoder. We start at READ_FIXED_HEADER, followed by READ_VARIABLE_HEADER and
   * finally READ_PAYLOAD.
   */
  enum DecoderState {
    READ_FIXED_HEADER,
