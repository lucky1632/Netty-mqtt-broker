...
/**
 * Encodes Mqtt messages into bytes following the protocol specification v3.1 as described here <a
 * href="http://public.dhe.ibm.com/software/dw/webservices/ws-mqtt/mqtt-v3r1.html">MQTTV3.1</a>
 */
...
public class MqttEncoder extends MessageToMessageEncoder<MqttMessage> {
  ...
  static ByteBuf doEncode(ByteBufAllocator byteBufAllocator, MqttMessage message) {
    ...
    switch (message.fixedHeader().messageType()) {
      ...
      case PUBREL:
      case PUBCOMP:
        return encodeMessageWithOnlySingleByteFixedHeaderAndPacketId(byteBufAllocator, message);
  ...
  private static ByteBuf encodeConnectMessage(
      ByteBufAllocator byteBufAllocator, MqttConnectMessage message) {
    ...
    MqttVersion mqttVersion =
        MqttVersion.fromProtocolNameAndLevel(
            variableHeader.protocolName(), (byte) variableHeader.protocolLevel());
    ...
    byte[] willTopicBytes =
        willTopic != null ? encodeStringUtf8(willTopic) : EmptyArrays.EMPTY_BYTES;
    String willMessage = payload.willMessage();
    byte[] willMessageBytes =
        willMessage != null ? encodeStringUtf8(willMessage) : EmptyArrays.EMPTY_BYTES;
  ...
  private static int getConnVariableHeaderFlag(MqttConnectVariableHeader variableHeader) {
    ...
    if (variableHeader.willRetain()) {
      flagByte |= 0x20;
    }
    flagByte |= (variableHeader.willQos().value() & 0x03) << 3;
    if (variableHeader.willFlag()) {
      flagByte |= 0x04;
    }
  ...
  private static ByteBuf encodeConnAckMessage(
      ByteBufAllocator byteBufAllocator, MqttConnAckMessage message) {
  ...
  private static ByteBuf encodeSubscribeMessage(
      ByteBufAllocator byteBufAllocator, MqttSubscribeMessage message) {
    ...
    for (MqttTopicSubscription topic : payload.subscriptions()) {
      ...
      buf.writeByte(topic.requestedQos().value());
    }
  ...
  private static ByteBuf encodeUnsubscribeMessage(
      ByteBufAllocator byteBufAllocator, MqttUnsubscribeMessage message) {
    int variableHeaderBufferSize = 2;
    int payloadBufferSize = 0;
  ...
  private static ByteBuf encodeSubAckMessage(
      ByteBufAllocator byteBufAllocator, MqttSubAckMessage message) {
    int variableHeaderBufferSize = 2;
  ...
  private static ByteBuf encodePublishMessage(
    ...
    int variableHeaderBufferSize =
        2 + topicNameBytes.length + (mqttFixedHeader.qos().value() > 0 ? 2 : 0);
  ...
  private static ByteBuf encodeMessageWithOnlySingleByteFixedHeaderAndPacketId(
    ...
    MqttFixedHeader mqttFixedHeader = message.fixedHeader();
    MqttPacketIdVariableHeader variableHeader =
